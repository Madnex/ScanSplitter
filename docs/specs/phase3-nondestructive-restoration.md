...
Project settings add three booleans, defaulting false for new and old manifests:
...
validation (`400` for unknown values); the three restoration settings themselves
...
- `tests/test_restoration.py`: conservative deskew, OpenCV 4/5 result-shape
  compatibility, color/fade behavior, pipeline opt-in behavior, and
  non-generative upscale.
...
Source-file mutation, generative reconstruction or colorization, automatic
dust/scratch inpainting, manual brush retouching, and preservation of per-box
overrides across re-detection.
